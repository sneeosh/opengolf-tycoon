...
func advance_to_next_day() -> void:
	...
	# Seasonal event advance notifications (2-day lookahead)
	_notify_upcoming_events()

func _notify_upcoming_events() -> void:
	var upcoming = SeasonalEvents.get_upcoming_events(current_day, 2)
	for entry in upcoming:
		var event: SeasonalEvents.SeasonEvent = entry.event
		var days_until: int = entry.days_until
		if days_until == 1:
			EventBus.notify("%s tomorrow!" % event.name, "info")
		elif days_until == 2:
			var mods: Array = []
			if event.spawn_modifier != 1.0:
				mods.append("%.0f%% golfer traffic" % (event.spawn_modifier * 100))
			if event.revenue_modifier != 1.0:
				mods.append("%.0f%% revenue" % (event.revenue_modifier * 100))
			var mod_text = " — expect %s" % ", ".join(mods) if not mods.is_empty() else ""
			EventBus.notify("%s in 2 days%s" % [event.name, mod_text], "info")

	# Also notify about today's active event
	var active = SeasonalEvents.get_active_event(current_day)
	if active:
		var mods: Array = []
		if active.spawn_modifier != 1.0:
			var pct = int((active.spawn_modifier - 1.0) * 100)
			mods.append("%+d%% golfers" % pct)
		if active.revenue_modifier != 1.0:
			var pct = int((active.revenue_modifier - 1.0) * 100)
			mods.append("%+d%% revenue" % pct)
		var mod_text = " (%s)" % ", ".join(mods) if not mods.is_empty() else ""
		EventBus.notify("%s today!%s" % [active.name, mod_text], "success")
...
class DailyStatistics:
	...
	func calculate_operating_costs(terrain_cost: int, hole_count: int, building_costs: int = 0) -> void:
		# Terrain maintenance from actual tiles, scaled by blended seasonal modifier
		var season_mod = SeasonSystem.get_blended_maintenance_modifier(GameManager.current_day, GameManager.current_theme)
