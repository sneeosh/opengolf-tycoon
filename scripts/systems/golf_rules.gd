...
static func get_lie_modifier(terrain_type: int, club: int) -> float:
	match terrain_type:
		...
		TerrainTypes.Type.TEE_BOX:
			return 1.05 if club == 0 else 1.0  # Slight tee bonus for driver
		...
		TerrainTypes.Type.BUNKER:
			# Wedges handle sand better (club index 3 = WEDGE)
			return 0.6 if club == 3 else 0.4
...
## Maps to Golfer.Club enum: DRIVER=0, FAIRWAY_WOOD=1, IRON=2, WEDGE=3, PUTTER=4.

const CLUB_WIND_SENSITIVITY: Dictionary = {
	0: 1.0,   # DRIVER - high trajectory, long hang time, full wind effect
	1: 0.85,  # FAIRWAY_WOOD - slightly lower trajectory than driver
	2: 0.7,   # IRON - medium trajectory
	3: 0.4,   # WEDGE - high but short, moderate wind effect
	4: 0.0,   # PUTTER - ground ball, no wind effect
}

static func get_club_wind_sensitivity(club: int) -> float:
	return CLUB_WIND_SENSITIVITY.get(club, 0.5)
