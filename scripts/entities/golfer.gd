...
func _handle_hazard_penalty(previous_position: Vector2i) -> bool:
	...
	match relief_type:
		GolfRules.ReliefType.DROP_AT_ENTRY:
			...
			var drop_position = _find_drop_position(ball_position)
...
## Find a valid drop position near a non-playable area, no closer to the hole
func _find_drop_position(water_position: Vector2i) -> Vector2i:
	var terrain_grid = GameManager.terrain_grid
	if not terrain_grid:
