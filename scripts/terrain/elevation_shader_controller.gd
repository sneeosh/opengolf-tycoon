...
##
## Updates camera mapping uniforms each frame and syncs light direction
## with the day/night cycle and weather conditions.
...
func _update_light_from_time() -> void:
	...
	if hour >= 6.0 and hour <= 18.0:
		...
		var angle := lerp(-PI * 0.15, -PI * 0.85, t)  # East to west arc
