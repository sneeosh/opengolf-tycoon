...
func test_fairway_perfect_lie() -> void:
	assert_eq(GolfRules.get_lie_modifier(TerrainTypes.Type.FAIRWAY, 2), 1.0)

func test_tee_box_driver_bonus() -> void:
	assert_gt(GolfRules.get_lie_modifier(TerrainTypes.Type.TEE_BOX, 0), 1.0,
		"Driver from tee should get slight bonus")

func test_rough_penalty() -> void:
	assert_lt(GolfRules.get_lie_modifier(TerrainTypes.Type.ROUGH, 2), 1.0)

func test_heavy_rough_worse_than_rough() -> void:
	var rough = GolfRules.get_lie_modifier(TerrainTypes.Type.ROUGH, 2)
	var heavy = GolfRules.get_lie_modifier(TerrainTypes.Type.HEAVY_ROUGH, 2)
	assert_lt(heavy, rough, "Heavy rough should be harder than rough")

func test_bunker_wedge_easier() -> void:
	var wedge = GolfRules.get_lie_modifier(TerrainTypes.Type.BUNKER, 3)  # WEDGE
	var iron = GolfRules.get_lie_modifier(TerrainTypes.Type.BUNKER, 2)   # IRON
	assert_gt(wedge, iron, "Wedge should be easier from bunker than iron")

func test_trees_very_difficult() -> void:
	assert_lte(GolfRules.get_lie_modifier(TerrainTypes.Type.TREES, 2), 0.3)
...
func test_driver_full_wind() -> void:
	assert_eq(GolfRules.get_club_wind_sensitivity(0), 1.0)  # DRIVER

func test_fairway_wood_high_wind() -> void:
	var fw = GolfRules.get_club_wind_sensitivity(1)  # FAIRWAY_WOOD
...
func test_iron_moderate_wind() -> void:
	var iron = GolfRules.get_club_wind_sensitivity(2)  # IRON
	assert_gt(iron, 0.3)
	assert_lt(iron, 0.9)

func test_wedge_low_wind() -> void:
	var wedge = GolfRules.get_club_wind_sensitivity(3)  # WEDGE
...
func test_putter_no_wind() -> void:
	assert_eq(GolfRules.get_club_wind_sensitivity(4), 0.0)  # PUTTER

func test_wind_sensitivity_decreasing_order() -> void:
	var driver = GolfRules.get_club_wind_sensitivity(0)
	var fw = GolfRules.get_club_wind_sensitivity(1)
	var iron = GolfRules.get_club_wind_sensitivity(2)
	var wedge = GolfRules.get_club_wind_sensitivity(3)
	var putter = GolfRules.get_club_wind_sensitivity(4)
